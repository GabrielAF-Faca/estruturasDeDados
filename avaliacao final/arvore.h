...
Arvore *arvore_inserir_sem_repetir(int valor, Arvore *r);
...
Arvore *arvore_excluir_multiplos_folhas(int valor, Arvore *raiz);
...
Arvore *arvore_excluir_multiplos_folhas(int valor, Arvore *raiz) {
    if (raiz) {
        if (arvore_eh_folha(raiz)) { 
            
            if (raiz->dado % valor == 0){
                cout << "Multiplo de "<< valor << " encontrado (" << raiz->dado << ")" << endl;

                free(raiz);
                return NULL;
            }
        } else {
            raiz->esq = arvore_excluir_multiplos_folhas(valor, raiz->esq);
            raiz->dir = arvore_excluir_multiplos_folhas(valor, raiz->dir);
        }
...
Arvore *arvore_inserir_sem_repetir(int valor, Arvore *r) {
    ...
    if (r) {
        ...
        if(valor < r->dado){
            r->esq = arvore_inserir_sem_repetir(valor, r->esq);

        } else {
            r->dir = arvore_inserir_sem_repetir(valor, r->dir);
...
int arvore_contar_vezes(int valor, Arvore *r) {
    ...
    if (r) {
        int qtdVezesEsquerda = arvore_contar_vezes(valor, r->esq);//empilhar tudo para esquerda
        int qtdVezesDireita = arvore_contar_vezes(valor, r->dir);//empilhar tudo para direita

        //so então testar, ou seja, o teste vem das folhas em direção a raiz
        if (valor == r->dado) return 1 + qtdVezesEsquerda + qtdVezesDireita;
